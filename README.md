...
## Highlights
- **Storytelling-ready forecasts** – glassmorphism-inspired UI, real-time charting, AI-generated insights, and bilingual copy throughout the workflow.
- **Adaptive modelling** – automatic switching between historical and metadata LightGBM pipelines based on uploaded sales history.
- **Human-in-the-loop AI** – NVIDIA Llama 3.3 for CSV mapping and insights, Nemotron Ultra for product validation, and Qwen 2.5-VL for image metadata extraction.
- **Offline-friendly operations** – JSON-based fallbacks in `.data/` ensure forecasting, history, and settings persist even without Supabase credentials.
- **Secure, scalable foundation** – Supabase handles auth, storage, and Postgres; optional Hugging Face Space keeps long-running inference outside Vercel limits.
- **Currency-aware cost handling** – costs are captured in any configured currency and converted to USD before modelling to match training data.
...
| **AI integrations** | CSV & form intelligence, insights, metadata | NVIDIA APIs and Hugging Face router. Optional direct Hugging Face Space call using `NEXT_PUBLIC_FORECAST_SERVICE_URL`. |
...
| `HF_TOKEN` | Hugging Face API token (Qwen 2.5-VL router) |
| `NVIDIA_API_KEY` | Access key for NVIDIA Llama and Nemotron services |
...
4. Update `DEPLOYMENT_GUIDE.md` and `QWEN_INTEGRATION.md` with project-specific instructions as you iterate.
...
Let’s build breathtaking forecasts. 🪄
