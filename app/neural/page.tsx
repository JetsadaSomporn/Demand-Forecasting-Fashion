...
export default function NeuralPage() {
  ...
  const handleSubmit = async (textOverride?: string) => {
    ...
    if (isWebSearch) fullContent = "[WEB SEARCH MODE] " + fullContent;
    ...
    <div className="fixed inset-0 w-full flex flex-col bg-white text-zinc-900 font-sans selection:bg-blue-100 overflow-hidden">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 bg-white/80 backdrop-blur-md">
         <div className="flex items-center gap-4">
             <button onClick={() => setIsSidebarOpen(true)} className="p-2 hover:bg-zinc-100 rounded-lg transition-all text-zinc-600">
                 <Menu className="w-5 h-5" />
             </button>
             <div className="relative">
                 <button onClick={() => setIsModelMenuOpen(!isModelMenuOpen)} className="flex items-center gap-2 px-3 py-1.5 hover:bg-zinc-100 rounded-lg transition-all font-medium text-zinc-800 text-sm">
                     <span>Neural-4</span>
                     <ChevronDown className={clsx("w-4 h-4 text-zinc-400 transition-transform", isModelMenuOpen && "rotate-180")} />
                 </button>
                 {isModelMenuOpen && (
                     <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-zinc-100 p-1.5 z-50 animate-in fade-in slide-in-from-top-2">
         ...
         <div className="relative">
            <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="w-9 h-9 rounded-full bg-gradient-to-tr from-zinc-100 to-zinc-200 flex items-center justify-center border border-zinc-200 overflow-hidden hover:shadow-md transition-all active:scale-95">
                <User className="w-5 h-5 text-zinc-600" />
            </button>
            {isUserMenuOpen && (
                <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-zinc-100 p-2 z-50 animate-in fade-in slide-in-from-top-2">
                     ...
                     <button onClick={() => router.push('/settings')} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm text-zinc-600 hover:bg-zinc-50 transition-colors">
      ...
      {/* --- Sidebar --- */}
      <div className={clsx("fixed inset-0 z-50 bg-black/20 backdrop-blur-sm transition-opacity", isSidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none")} onClick={() => setIsSidebarOpen(false)}>
            <div className={clsx("absolute inset-y-0 left-0 w-72 bg-white shadow-2xl p-4 flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]", isSidebarOpen ? "translate-x-0" : "-translate-x-full")} onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-8 px-2 pt-2">
                    <h2 className="font-semibold text-zinc-800 tracking-tight text-lg">Your Chats</h2>
                    <button onClick={() => router.push('/forecast')} className="p-2 hover:bg-zinc-100 rounded-lg text-zinc-500 transition-colors">
                         <LayoutGrid className="w-5 h-5" />
                    </button>
                </div>
                <button onClick={startNewChat} className="flex items-center gap-3 w-full p-3.5 bg-zinc-900 text-white rounded-xl mb-6 hover:bg-zinc-800 transition-all justify-center font-medium text-sm">
                    <Plus className="w-4 h-4" />
                    Start New Chat
                </button>
                <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar -mx-2 px-2">
                    {sessions.map(s => (
                        <button key={s.id} onClick={() => loadSession(s.id)} className={clsx("w-full text-left p-3 rounded-lg text-sm truncate transition-all", sessionId === s.id ? "bg-zinc-100 text-zinc-900 font-medium" : "text-zinc-600 hover:bg-zinc-50")}>
      ...
      {/* --- Main --- */}
      <main className="flex-1 flex flex-col relative w-full max-w-4xl mx-auto pt-16 overflow-hidden">
          
          {messages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center w-full px-4 animate-in fade-in zoom-in-[0.98] duration-700">
                  <h1 className="text-4xl md:text-5xl font-bold mb-10 text-transparent bg-clip-text bg-gradient-to-br from-zinc-900 via-zinc-700 to-zinc-500 text-center tracking-tight">
                      Hi, {userName}
                  </h1>
                  ...
                  <div className="w-full max-w-2xl bg-white rounded-2xl border border-zinc-200 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.1)] p-4 transition-all group">
                      <div className="flex items-center gap-2 mb-3 px-1">
                          ...
                          <button onClick={() => setIsReasoning(!isReasoning)} className={clsx("flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold transition-all", isReasoning ? "bg-blue-50 text-blue-600 ring-1 ring-blue-100" : "text-zinc-500 hover:bg-zinc-100")}>
                      ...
                      {files.length > 0 && (
                          <div className="flex gap-2 mb-3 overflow-x-auto pb-2 custom-scrollbar">
                              {files.map((f, i) => (
                                  <div key={i} className="flex items-center gap-2 bg-zinc-50 border border-zinc-100 rounded-lg px-3 py-2 text-xs text-zinc-700 shrink-0">
                                      <FileText className="w-3.5 h-3.5 text-blue-500" />
                                      <span className="truncate max-w-[120px] font-medium">{f.name}</span>
                                      <button onClick={() => removeFile(i)} className="hover:bg-zinc-200 p-0.5 rounded-full transition-colors"><X className="w-3 h-3" /></button>
                                  </div>
                              ))}
                          </div>
                      )}
                  ...
                  <div className="mt-8 flex flex-wrap justify-center gap-3">
                      {[ 
                          ...
                          <button key={i} onClick={() => { setInput(item.prompt); textareaRef.current?.focus(); }} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-zinc-200 rounded-full text-sm text-zinc-600 hover:border-zinc-300 hover:shadow-md transition-all">
              ...
              <div className="flex-1 flex flex-col min-h-0 w-full relative">
                  <div className="flex-1 overflow-y-auto px-4 pb-36 pt-8 space-y-8" ref={scrollRef}>
                      {messages.map((msg, idx) => (
                          <div key={idx} className={clsx("flex gap-5 max-w-3xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500", msg.role === "user" ? "justify-end" : "justify-start")}>
                                {msg.role === "assistant" && (
                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shrink-0 mt-1 shadow-md">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white/95 to-transparent z-20">
                      ...
                      <div className="text-center mt-2 text-[10px] text-zinc-400 font-medium">Neural can make mistakes. Check important info.</div>
                  </div>
              </div>
          )}
      </main>
